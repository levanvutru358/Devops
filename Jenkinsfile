      ...
      steps {
        withCredentials([
          ...
          sh '''
            ...
            # Use scp with quiet mode and proper SSH options
            scp -i "$SSH_KEY" -o StrictHostKeyChecking=no -o LogLevel=ERROR -q docker-compose.yml $SSH_USER@$SERVER_HOST:~/project/
            
            echo "Deploying on server..."
            ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no -o LogLevel=ERROR -o BatchMode=yes $SSH_USER@$SERVER_HOST '
              cd ~/project &&
